...
func CmdReceiverStoreConfigChangeInfo() error {
	...
	queueName := queuePrefix + configs.StoreConfigQueueName
	...
	go func() {
		for d := range msgs {
			...
			queErr := rabbitmq.NewStoreConfigChangeInfoReceiver(&d)
			if queErr != nil {
				zlog.Error().Msgf("NewAppOrderReceiver Err %s", queErr.Error())
			}
